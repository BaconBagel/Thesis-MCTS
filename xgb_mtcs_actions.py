...
def transition(state, action):
    ...
    reward = rewards[next_t, x * 100 + y] + (0.2*rand_mult*rewards[next_t, x * 100 + y])
    ...
    reward_l, reward_r, reward_u, reward_d, reward_dr, reward_ur, reward_ul, reward_dl = 0, 0, 0, 0, 0, 0, 0, 0

    if reward_horizon > 0:
        for n in range(1, reward_horizon):
            # check boundaries
            if n < x < 99 - n and n < y < 99 - n:
                random_number = 1 + (random.random()*0.2)  # Generate a random number between 0 and 1

                reward_l += random_number * max(rewards[next_t, (x - n) * 100 + y] - (((0.4 * n) / speed) * 3600), 0)
                reward_r += random_number * max(rewards[next_t, (x + n) * 100 + y] - (((0.4 * n) / speed) * 3600), 0)
                reward_u += random_number * max(rewards[next_t, x * 100 + (y + n)] - (((0.4 * n) / speed) * 3600), 0)
                reward_d += random_number * max(rewards[next_t, x * 100 + (y - n)] - (((0.4 * n) / speed) * 3600), 0)
                reward_dr += random_number * max(
                    rewards[next_t, (x - n) * 100 + (y - n)] - (((0.4 * n) / speed) * 3600), 0)
                reward_dl += random_number * max(
                    rewards[next_t, (x + n) * 100 + (y - n)] - (((0.4 * n) / speed) * 3600), 0)
                reward_ul += random_number * max(
                    rewards[next_t, (x - n) * 100 + (y + n)] - (((0.4 * n) / speed) * 3600), 0)
                reward_ur += random_number * max(
                    rewards[next_t, (x - n) * 100 + (y + n)] - (((0.4 * n) / speed) * 3600), 0)

    reward = reward + reward_l + reward_r + reward_u + reward_d + reward_dr + reward_ur + reward_ul + reward_ur
...
# Generate x-coordinates (assuming it's a simple sequence from 0 to 999)
print(sum(reward_sequences)/len(reward_sequences))
"""
