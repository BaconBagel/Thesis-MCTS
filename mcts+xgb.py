...
for o in range(0, 99):
    ...
    for i in range(571):
        ...
        nxt, rwd = transition(nxt, 10, rewards)
        sum_rwd += rwd
    reward_sequences.append(sum_rwd/(571*10))
...
for o in range(0, 99):
    ...
    for i in range(571):
        ...
        nxt, rwd = transition(nxt, 10, rewards)
        sum_rwd += rwd
    reward_sequences2.append(sum_rwd/(571*10))
...
for o in range(0, 99):
    ...
    for i in range(571):
        ...
        nxt, rwd = transition(nxt, 10, rewards)
        sum_rwd += rwd
    reward_sequences3.append(sum_rwd/(571*10))
...
# Compute regression for reward_sequences
# Compute regression and residuals for reward_sequences


x_data = np.arange(len(reward_sequences))
popt, _ = curve_fit(sine_func, x_data, reward_sequences, maxfev=50000, absolute_sigma=False)
y_fit = sine_func(x_data, *popt)
residuals = reward_sequences - y_fit
ssr1 = np.sum(residuals**2)

# Compute regression and residuals for reward_sequences2
x_data2 = np.arange(len(reward_sequences2))
popt2, _ = curve_fit(sine_func, x_data2, reward_sequences2, maxfev=50000,absolute_sigma=False)
y_fit2 = sine_func(x_data2, *popt2)
residuals2 = reward_sequences2 - y_fit2
ssr2 = np.sum(residuals2**2)

# Compute regression and residuals for reward_sequences3
x_data3 = np.arange(len(reward_sequences3))
popt3, _ = curve_fit(sine_func, x_data3, reward_sequences3, maxfev=50000, absolute_sigma=False)
y_fit3 = sine_func(x_data3, *popt3)
residuals3 = reward_sequences3 - y_fit3
...
plt.ylabel('Normalised eduction Potential (seconds per timestep)')
plt.legend()
plt.show()
"""
